...
public class RelatedDocumentsFromSolrWithKeyphrases extends RelatedDocumentsFromSolr {
	public RelatedDocumentsFromSolrWithKeyphrases() throws Exception {
		...
		if(sum == 3){
			...
			if(!unigrams){
				name = "bitri";
			}else name = bigrams?"unibi":"unitri";
		}else{
			if(unigrams){
				name = "unigrams";
			}else name = bigrams?"bigrams":"trigrams";
		}
	...
	public DocumentSet getRelatedDocumentSet(DisplayDocument requestDoc, int numberOfRelatedDocs) throws Exception {
		try {
			int maxNumber = con.getMinimumNumberOfKeyphrases(requestDoc.getDocumentId(),loggingInfo.get("typeOfGram"), loggingInfo.get("cbf_text_fields"));
			if(maxNumber<1) throw new NoRelatedDocumentsException(requestDoc.getOriginalDocumentId(),requestDoc.getDocumentId());
			Random random = new Random();
